...
m_pixelY(pixelY),
m_color(newColor),
m_prevColor(image.getPixel(pixelX, pixelY)),
m_ptrImage(&image){
}
...
: Command("Draw"),
m_pixelX(obj.m_pixelX),
m_pixelY(obj.m_pixelX),
m_color(obj.m_color),
m_prevColor((*obj.m_ptrImage).getPixel(obj.m_pixelX, obj.m_pixelY)),
m_ptrImage(obj.m_ptrImage){
}

/*! \brief 	The isEqual method compares the command with another command object.
*		If the command has the same pixel location and color, the bool value true will be
